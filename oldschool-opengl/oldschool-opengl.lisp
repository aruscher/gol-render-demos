        ...
        (dotimes (row rows)
          (dotimes (col cols)
            (let ((top-left (list (* col cell-width) (* row cell-height)))
                  (top-right (list (+ (* col cell-width) cell-width) (* row cell-height)))
                  (down-right (list (+ (* col cell-width) cell-width) (+ (* row cell-height) cell-height)))
                  (down-left ( list (* col cell-width) (+ (* row cell-height) cell-height))))
              (if (= (get-cell (game-of-life w) row col) 1)
                                   (%gl:color-3f 1.0 1.0 1.0)
                                   (%gl:color-3f 0.0 0.0 0.0))
              (gl:vertex (first top-left) (second top-left))
              (gl:vertex (first top-right) (second top-right))
              (gl:vertex (first down-right) (second down-right))
              (gl:vertex (first down-left) (second down-right))
              ))))))
